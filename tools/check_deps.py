...
# Движок окна. Его НЕТ в импортах нашего кода — его импортирует сам pywebview,
# уже внутри себя. Поэтому обычная сверка «что импортируем» его пропускает, и
# сборка без бэкенда выглядела зелёной.
#
# ЗАМЕРЕНО на прогоне 1.1.5: Linux-сборка прошла проверку зависимостей, а у
# пользователя умерла с «You must have either QT or GTK with Python extensions
# installed». Без этой проверки дыра повторится.
БЭКЕНД_ОКНА = {
    "win32": ["clr"],          # pythonnet, через него работает WebView2
    "darwin": ["objc"],        # pyobjc, через него работает WKWebView
    "linux": ["PyQt6"],        # Qt WebEngine уезжает внутрь сборки
}


def нужный_бэкенд(папка):
    """Какой движок окна обязан лежать в сборке ДЛЯ ЭТОЙ системы."""
    for ключ, модули in БЭКЕНД_ОКНА.items():
        if sys.platform.startswith(ключ):
            return модули
    return []
...
def main():
    ...
    # Движок окна проверяется отдельно: его импортирует pywebview у себя
    # внутри, и в импортах нашего кода его нет.
    print()
    плохо_бэкенда = 0
    нужны_бэкенды = нужный_бэкенд(папка)
    print("движок окна для %s: %s" % (sys.platform, нужны_бэкенды or "не задан"))
    for м in нужны_бэкенды:
        есть_он = м in есть
        if not есть_он:
            плохо_бэкенда += 1
        print("  %s %-18s %s" % ("ok  " if есть_он else "НЕТ ", м,
                                 "найден в сборке" if есть_он
                                 else "В СБОРКЕ НЕТ — окно не откроется"))
    if плохо_бэкенда:
        print("ПРОВАЛ: движка окна в сборке нет, программа не покажет окно")
        return 1
