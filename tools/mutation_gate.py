...
MUTATIONS = [
    {
        "id": "M1",
        "title": "синтаксис: перестать отвергать двойные точки и пробелы",
        "path": "core/email_syntax.py",
        "old": "        if _BAD_SYNTAX_PATTERNS.search(email):\n            return False",
        "new": "        if _BAD_SYNTAX_PATTERNS.search(email):\n            pass",
        # Сторож ровно один и он точный: латинские случаи ловит ещё и общая
        # регулярка dot-atom, поэтому мутацию видит только проверка НЕ-ASCII
        # имени ящика — у неё второго рубежа нет. Прежние три файла её не
        # замечали, и гейт каждый раз добирал ответ полным прогоном набора.
        "guards": ["tests/test_syntax_bytes.py"],
        "why": "мусор уехал бы в проверку по сети как обычный адрес",
    },
    {
        "id": "M2",
