...
def _версия_от_программы(exe, ждать=60):
    """Спрашивает версию у запущенной программы ключом --version.

    Ответ идёт в файл: у сборки с console=False потоков вывода нет, и печать
    было бы некуда деть. Возвращает строку версии либо объяснение провала.
    """
    import subprocess
    import tempfile
...
def запустить_и_посмотреть(exe, ждать=45):
    ...
    try:
        ...
        # ВЕРСИИ В ЗАГОЛОВКЕ БЫТЬ НЕ ДОЛЖНО — решение владельца 11.09.2026:
        # в софте её нигде явно не показывать. Проверка не убрана, а
        # ПЕРЕВЁРНУТА: раньше требовала версию в заголовке, теперь требует её
        # отсутствия. Молча выкинуть пункт значило бы потерять место, где это
        # решение вообще кем-то сторожится.
        версия = open(os.path.join(КОРЕНЬ, "VERSION"), encoding="utf-8").read().strip()
        шаг(версия not in заголовок,
            "версии в заголовке нет (в нём %r)" % заголовок)

        # А сама версия сверяется у ЗАПУЩЕННОЙ программы — ключом, который
        # человеку не показывается. Пятое место сверки версий никуда не
        # делось, оно лишь перестало проходить через глаза пользователя.
        сказано = _версия_от_программы(exe)
        print("     программа назвала версию: %r" % сказано)
        шаг(сказано == версия, "программа называет %s" % версия)
