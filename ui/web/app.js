...
function onVisibilityChange() {
  if (document.visibilityState === "hidden") {
    // СЮДА НИЧЕГО НЕ ПИШЕТСЯ. Раньше здесь стоял вызов reportClientCrash, и
    // это была ложная тревога чистой воды: `hidden` наступает, когда окно
    // СВЁРНУТО или владелец переключился на другое приложение. Нормальнее
    // события не бывает.
    //
    // Замерено в data/crash.log: три записи за сутки, и все три — эта
    // строка. Настоящих сбоев ноль. При следующем запуске она же порождала
    // красную строку про подавление и вторую про «сбои за сутки», то есть
    // программа пугала владельца ровно тем, что он свернул окно.
    //
    // Настоящее подавление ловится не отсюда, а сторожем в webapp.py: он
    // сравнивает ЧАСТОТУ тиков с ожидаемой, пока страница считает себя
    // видимой. Вот это действительно ненормально, и вот об этом стоит
    // говорить.
    return;
  }
...
// Строка «продолжить прерванный прогон» из окна убрана: продолжение
// включено в ядре всегда и стирается само после прогона, дошедшего до
// конца (см. core/pipeline.py). Спрашивать владельца стало не о чем.
...
const SOURCE_TEXT = {
  ...
  "разбор": "склеено сегментатором — догадка",
  "профиль": "из публичного профиля — факт",
...
/* Вставка текстом.
   ...
   Примеры адресов показывают ПОРЯДОК и КОМБИНАЦИИ полей, а не одни лишь
   разделители: почта первой, последней и посередине, четыре поля, три, два,
   одно. Ради этого переписывался разбор строки, и по одному примеру со
   всеми полями подряд этого было не видно.

   Про «разделитель один на список» сказано не для красоты. ЗАМЕРЕНО: если
   вставить блок со смешанными разделителями, определится ОДИН — тот, что
   чаще, — и у остальных строк имя, пол и страна молча пропадут. Сами
   примеры ниже намеренно разные: они показывают принимаемые формы, а не
   готовый к вставке список. */
const PASTE_FORMATS = {
  emails: {
    title: "Вставьте список адресов",
    hint: "Одна запись в строке. Кроме адреса можно дать имя, пол и страну — " +
          "в любом порядке и любой комбинации; обязателен только адрес. " +
          "Разделитель любой из , ; : | или табуляция, но в одном списке он " +
          "должен быть ОДИН. Пол и страна понимаются и по-русски, и " +
          "по-английски. Первая строка может быть заголовком CSV.",
    placeholder: [
      "ivan@gmail.com",
      "anna@yahoo.com;Анна;Женский;США",
      "Пётр Смирнов;petr@mail.ru;Мужской;Россия",
      "Россия;Мужской;Ольга;olga@outlook.com",
      "sergey@bk.ru;Сергей",
      "США;maria@hotmail.com",
      "maria@yandex.ru;;Женский;Россия",
      "john@aol.com;John Smith;male;USA",
      "nina@list.ru,Нина,Женский",
      "oleg@rambler.ru|Олег|Мужской|Россия",
...
/* Подпись под переключателем «Имя» и «Страна».

   ПОЧЕМУ ОТДЕЛЬНОЙ ТАБЛИЦЕЙ, А НЕ ТЕРНАРНИКОМ НА МЕСТЕ. Текст жил в двух
   местах сразу: один вариант был зашит в разметку, второй подставлялся при
   щелчке. При открытии окна подсвечена «Точность», а под ней читалось
   описание «Заполненности» — то есть окно обещало треть выдуманных стран
   там, где их 8.8%. По этой колонке принимаются решения о сегментации
   рассылки, так что цена расхождения не косметическая.

   Пока текстов два, они расходятся снова при любой смене режима по
   умолчанию. Теперь источник один и он же рисует подпись на старте. */
const COUNTRY_HINT = {
  accuracy: "имя ставится только когда словарь его подтвердил, страна — только при уверенном распределении (заполнено ~47% против ~70%)",
  coverage: "заполнено почти всегда: ~треть стран догадка, имя может быть склеено сегментатором из чего угодно",
};

function renderCountryHint() {
  setText($("#countryHint"), COUNTRY_HINT[ui.country] || "");
}
...
$("#btnStart").addEventListener("click", async () => {
  const payload = {
    ...
    country: ui.country,
    // Пяти настроек качества здесь больше НЕТ. Они включены значениями по
    // умолчанию в ядре, и окно их не трогает вовсе. Слать отсюда false —
    // ровно то, чего владелец просил избежать: он не хочет включать их
    // руками перед каждым прогоном.
  };
