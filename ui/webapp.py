...
class WindowWatchdog:
    ...
    def check_once(self, now=None):
        ...
        # В ЖУРНАЛ ОКНА НЕ ПИШЕМ. Решение владельца 11.09.2026: «не надо мне
        # лишние логи отображать». Это самодиагностика сторожа — она
        # появляется сама, когда человек ничего не делал, и объяснить ему
        # ничего не может. Запись в crash.log выше остаётся: разбирать сбой
        # по-прежнему есть по чему, просто разбор не лезет человеку в глаза.
        return True
    ...
    def check_rate(self, now=None):
        ...
        # В журнал окна не пишем — по той же причине, что и в check_once.
        # Именно эту строку владелец показал на снимке как лишнюю.
        return True
...
def _announce_past_crashes(api, within_hours=24):
    ...
    if not found:
        return 0
    # В ЖУРНАЛ ОКНА НЕ ПИШЕМ ВОВСЕ.
    #
    # Путь этой строки: сначала было красное полотно из трёх строк, потом
    # одна строка с меткой INFO, теперь — ничего. Решение владельца
    # 11.09.2026: «не надо мне лишние логи отображать». И он прав по сути:
    # сообщение про ПРОШЛУЮ аварию встречает человека на пустом месте, когда
    # прямо сейчас ничего не сломалось, а сделать с ним он всё равно ничего
    # не может.
    #
    # Число возвращается по-прежнему: оно нужно проверкам и разбору. Сам
    # crash.log никуда не делся и пишется как раньше.
    return len(found)
...
def run(selftest_close=None):
    ...
    #
    # Решение владельца 11.09.2026: версию нигде в софте явно не показывать.
    # Сверка версий при этом НЕ ослабла: пятым местом вместо заголовка стал
    # ответ самой запущенной программы на ключ --version, см. main.py и
    # tools/check_version.py. Убрать место сверки было бы подгонкой проверки
    # под правку, а не выполнением просьбы.
