...
def group_of(status):
    ...
    # Всё остальное тоже «не доказано», и прятать его нельзя.
    #
    # Раньше сюда попадали Catch-All («сервер отвечает 250 на любой адрес,
    # проверить ящик невозможно») и Unverified («проверку не делали»), а группа
    # other не показана НИ ОДНИМ фильтром окна. Адрес проверялся, считался в
    # «Всего проверено» — и пропадал из таблицы: владелец видел 78 проверенных
    # и 77 строк, не понимая, куда делась одна.
    #
    # По смыслу оба случая — ровно «доказательства нет», то же самое, что
    # Risky и Unknown. Туда их и кладём: лучше показать адрес честно
    # недоказанным, чем спрятать совсем.
    return "unknown"


# Группа "other" оставлена ради совместимости индексов и старых выборок, но
# больше не наполняется: любой статус теперь попадает в одну из четырёх
# групп, которые окно действительно показывает.
...
class ResultStore:
    ...
    def _open(self, path):
        ...
        try:
            ...
            for name, column in (("rows_grp_country", "grp, country"),
                                 ...
                                 # Отбор сразу по нескольким граням — то, что
                                 # владелец делает руками: страна плюс пол
                                 # плюс порог скора. По одиночным индексам
                                 # SQLite берёт ОДИН из них, а остальное
                                 # проверяет перебором найденного: замерено на
                                 # миллионе строк — 0.4 с на каждое движение
                                 # фильтра, и всё это время хранилище под
                                 # замком. Этот индекс покрывает запрос
                                 # целиком (в него входят все колонки условия),
                                 # и ответ приходит за 0.019 с.
                                 ("rows_facets", "grp, country, gender, score"),
                                 # Покрывающий индекс под поиск по вхождению.
                                 # `LIKE '%иголка%'` начинается со звёздочки и
                                 # индексом воспользоваться не может — но
                                 # ПЕРЕБИРАТЬ он может либо таблицу, либо этот
                                 # индекс, а он в разы меньше: в нём только
                                 # группа и адрес, без имени, страны и причины.
                                 # Замерено на миллионе строк: 3.3 секунды по
                                 # таблице против 0.1 по индексу — и смысл
                                 # поиска при этом не меняется ни на йоту.
                                 ("rows_grp_email", "grp, email_lc")):
                self._conn.execute(
    ...
    def _where(self, filters):
        ...
        if filters["search"]:
            # Поиск ПО ВХОЖДЕНИЮ — именно так, как ожидает владелец: строка
            # ищется в любом месте адреса, а не только в начале.
            #
            # Быстрым его делает не урезание смысла, а покрывающий индекс
            # (grp, email_lc): SQLite перебирает его, а не таблицу, и читает
            # десятки мегабайт вместо сотен. Замерено на миллионе строк —
            # 3.3 секунды по таблице против 0.1 по индексу.
            #
            # Колонка email_lc уже в нижнем регистре: LOWER(email) в условии
            # выключил бы индекс.
            #
            # Проценты и подчёркивания экранируются, иначе поиск «a_b» нашёл
            # бы «axb». В SQLite обратный слэш сам по себе ничего не
            # экранирует, поэтому символ экранирования объявляется явно и
            # записывается ОДИН раз: ESCAPE '\\' задал бы escape-символом два
            # слэша, и защита процентов перестала бы работать.
