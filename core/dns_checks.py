...
class DnsChecksMixin:
    ...
    # Сколько выдуманных имён спросить, решая про wildcard. Одного мало:
    # имя может случайно существовать (у крупных хостингов бывают тысячи
    # поддоменов). Двух хватает: совпасть дважды подряд случайные строки из
    # шестнадцати знаков не могут.
    _WILDCARD_PROBES = 2

    def _is_wildcard_domain(self, domain):
        """Резолвится ли у домена ЛЮБОЕ выдуманное имя.

        Возвращает True только при УВЕРЕННОСТИ: обе пробы должны ответить.
        Сбой резолвера даёт False — объявить домен подстановочным из-за
        собственной неудачи значило бы выбросить его адреса, а это ровно та
        ошибка, от которой защищает весь остальной код.
        """
        import secrets

        for _ in range(self._WILDCARD_PROBES):
            выдуманное = "%s.%s" % (secrets.token_hex(8), domain)
            try:
                self.resolver.resolve(выдуманное, "A")
            except Exception:
    ...
    def get_mx_records(self, domain: str):
        ...
        # СБОЙ на MX закрывает вопрос сразу.
        #
        # Фоллбэк на A/AAAA осмыслен ровно в одном случае: сервер ОТВЕТИЛ, что
        # MX-записей нет. Если же он не ответил вовсе (таймаут, прокси, обрыв),
        # то настоящий список MX неизвестен — а подстановка самого домена
        # означает «шлём пробы на веб-сервер вместо почтового», и хуже того:
        # результат кэшируется на весь прогон, поэтому ошибка переживает
        ...
        #
        # НО СНАЧАЛА проверяем, не wildcard ли это. У домена с подстановочной
        # записью резолвится ЛЮБОЕ имя, включая заведомо выдуманное, и тогда
        # «у домена есть A-запись» не значит ровным счётом ничего: почтового
        # сервера там нет, а проба уйдёт на веб-сервер или на парковку.
        # Найдено сверкой с чужим каталогом критериев (4.22).
