...
# Слоги для ПРАВДОПОДОБНОГО имени ящика.
#
# Нужны отдельно от случайных букв, и вот почему. Обе прежние формы пробы —
# `r2e3s7k3xmu18951` и `f4118f38f4f14eafa3c9c1c6` — с первого взгляда машинные.
# Сервер, который защищается от перебора, отсеивает их по ВИДУ адреса, ни разу
# не заглянув в список ящиков, — и обе наши проверки читают этот отказ как
# «сервер отвечает честно». После чего КАЖДЫЙ настоящий адрес такого домена
# получает 250 просто за то, что похож на имя, и уезжает в Valid с ПОВЫШЕННОЙ
# уверенностью: контрольная проба ведь «была отвергнута».
#
# Имя из слогов такой фильтр не отличит от настоящего. Если сервер принял и
# его — он принимает всё подряд, и это ровно то, что проба обязана выяснить.
_СЛОГИ = ("ma", "ne", "ri", "lo", "ta", "ve", "si", "ko", "da", "le",
          "mi", "no", "ra", "se", "ti", "vo", "ba", "de", "li", "mo",
          "na", "re", "so", "tu", "va", "be", "di", "lu", "ka", "pe",
          "ni", "ro")
...
def _generate_random_local(style="short"):
    ...
    style='short' — классический (16 символов);
    style='uuid'  — UUID-подобный (п.4 Catch-All);
    style='name'  — правдоподобное имя из слогов: `marisolonetuve.dabekolinora4127`.

    У формы 'name' случайности с запасом: двенадцать слогов из тридцати двух
    плюс четыре цифры — это 32^12 * 10^4, около 10^22 вариантов. Совпасть с
    существующим ящиком на любом мыслимом почтовике она не может, иначе проба
    сама рождала бы ложный вывод «домен принимает что угодно».
    """
    if style == "uuid":
        import uuid
        return str(uuid.uuid4()).replace('-', '')[:24]
    if style == "name":
        часть = lambda: ''.join(random.choices(_СЛОГИ, k=6))   # noqa: E731
        цифры = ''.join(random.choices(string.digits, k=4))
        return f"{часть()}.{часть()}{цифры}"
...
def control_probe_address(domain):
    """Адрес для КОНТРОЛЬНОЙ пробы в той же сессии.

    Отдельной функцией, чтобы у неё был проверяемый договор: форма пробы —
    это не деталь реализации, а то, от чего зависит, поймаем ли мы врущий
    сервер. Договор проверяется в tests/test_lineaudit.py (дефект D-08).
    """
...
class NetworkValidator(ProxyPoolMixin, DnsChecksMixin):
    ...
    def __init__(self, from_email="check@example.com", timeout=5, proxies=None,
        ...
        # Существует ли ХОСТ из MX-записи. Отдельно от mx_cache: там ответ по
        # домену, здесь по имени сервера, и одно имя обслуживает тысячи
        # доменов — без своего кеша мы спрашивали бы про
        # `mta5.am0.yahoodns.net` сто тысяч раз за прогон.
        self._mx_host_cache = BoundedCache()
        self._mx_host_lock = threading.Lock()
    ...
    def _do_single_ping(self, email, mx_record, proxy=None, from_email=None,
        ...
        try:
            ...
            if control_probe and result["status"] == "valid" and domain:
                # Форма пробы — ПРАВДОПОДОБНОЕ ИМЯ, а не случайные буквы.
                # Почему это решает исход, а не украшает код, — у
                # control_probe_address и _СЛОГИ.
                fake = control_probe_address(domain)
                try:
    ...
    def check_email(self, email: str, avoid_exit_of=None,
        ...
        # Зарезервированная зона: домена НЕ СУЩЕСТВУЕТ и существовать не
        # может. `.test`, `.invalid`, `.local`, `.internal` и подобные в
        # корень интернета не делегированы (RFC 2606/6761/6762/8375, решение
        # ICANN 2024 по `.internal`).
        #
        # Это единственный вид приговора наравне с пустым именем ящика,
        # который выносится БЕЗ СЕТИ и при этом честен: тут нечего
        # перепроверять другим прокси или завтра. Раньше такой адрес уходил в
        # DNS, получал «домен не найден» и был неотличим от домена, до
        # которого просто не дозвонились.
        #
        # Синтаксис у него при этом ПРАВИЛЬНЫЙ, и говорить «неправильный
        # синтаксис» было бы враньём: причина другая и названа своими словами.
        зона = reserved_zone(domain)
        if зона:
            return {"status": "invalid",
                    "reason": ("Зона .%s зарезервирована стандартом и не "
                               "существует в интернете — почту такой домен "
                               "принять не может" % зона),
                    "mx_record": "N/A"}
        ...
        # Шаг 4.7: не достучались — а сервер из MX вообще существует?
        #
        # MX-запись живёт в DNS отдельно от сервера, на который указывает.
        # Домен съезжает с Microsoft 365, подписку закрывают, хост
        # `x-com.mail.protection.outlook.com` исчезает — а запись остаётся.
        # Такой домен выглядел живым: MX есть, стучимся, не достучались,
        # «не проверено». ЗАМЕРЕНО на 300 редких доменах базы владельца:
        # 3 домена (1.0%) именно такие, NXDOMAIN подтверждён тремя запросами
        # подряд при живом контроле.
        #
        # СПРАШИВАЕТСЯ ТОЛЬКО ЗДЕСЬ, а не сразу после получения MX. Разница в
        # цене: там это лишний запрос DNS на КАЖДЫЙ домен базы, здесь — только
        # на те, где почта и так не ответила. Ответ у нас всё равно ничего не
        # меняет, пока сессия удалась.
        #
        # Приговор выносится ТОЛЬКО на NXDOMAIN — на ответе «такого имени
        # нет». Таймаут, SERVFAIL и сбой прокси дают None, и тогда вердикт
        # остаётся прежним: хоронить домен за нашу собственную неудачу
        # нельзя (см. mx_hosts_alive в core/dns_checks.py).
        # Баннер означает, что сервер ОТВЕТИЛ, то есть существует. Спрашивать
        # после этого у DNS, есть ли он, бессмысленно, а поверить DNS против
        # живого ответа — прямая дорога к ложному приговору: шагом выше
        # вердикт мог быть намеренно понижен до risky (сервер отвергает
        # postmaster@, значит его отказам верить нельзя), и переписать это
        # обратно в invalid значило бы отменить защиту.
        if (литерал is None
                and result.get("status") in ("unknown", "risky")
                and not result.get("smtp_banner")
                and self.mx_hosts_alive(mx_records) is False):
            result["status"] = "invalid"
            result["reason"] = ("MX-запись есть, но её сервер не существует "
                                "(NXDOMAIN) — домен почту принять не может")
            result["mx_record"] = primary_mx
            result["mx_records"] = mx_records
            return result
