...
class NameExtractor:
    ...
    def _repair_oversegmentation(self, parts):
        ...
        знает, поэтому режет `kovbinbogdan` на ['kov','bin','bogdan'] — и
        адрес владельца превращался в «Kov Bin Bogdan». Проверить склейку по
        ...
        kovbin b), и склеивать её нельзя.

        ['kov','bin','bogdan']      -> ['kovbin','bogdan']
        ['kov','bin','b']           -> ['kovbin','b']
    ...
    def _finalize(self, parts, email, from_separators=False):
        ...
        if self.nd and not self._is_valid_name(name_to_check, role=role):
            ...
            if not self._is_valid_name(name_to_check, role="any"):
                ...
                # `kovbinbogdan1` -> Kovbin Bogdan, `kovbinb` -> Kovbin B:
