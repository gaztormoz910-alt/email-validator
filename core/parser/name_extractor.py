...
class NameExtractor:
    ...
    def _repair_oversegmentation(self, parts):
        ...
        Зачем. wordsegment обучен на английском корпусе и слова «kovbin» не
        знает, поэтому режет `kovbinpetr` на ['kov','bin','petr'] — и
        адрес человека превращался в «Kov Bin Petr». Проверить склейку по
        базе имён нельзя: базы на 138 млн записей «kovbin» тоже не знает,
        зато знает и «kov», и «bin» по отдельности — на такой выборке
        ...
        kovbin p), и склеивать её нельзя.

        Пример выдуманный, но поведение на нём замерено тем же кодом, что и
        на исходном случае из жизни: разбиение, склейка и итоговое имя
        совпадают. Настоящий адрес здесь стоять не может — эта строка
        уезжает в собранную программу и видна любому, кто её разберёт.

        ['kov','bin','petr']        -> ['kovbin','petr']
        ['kov','bin','p']           -> ['kovbin','p']
        ...
        for index, part in enumerate(parts):
            ...
            # обломок «kov», и её ответ здесь ничего не различает.
    ...
    def _finalize(self, parts, email, from_separators=False):
        ...
        if self.nd and not self._is_valid_name(name_to_check, role=role):
            ...
            if not self._is_valid_name(name_to_check, role="any"):
                ...
                # `kovbinpetr1` -> Kovbin Petr, `kovbinp` -> Kovbin P:
                # фамилии «kovbin» нет ни в базе на 138 млн, ни в индексе, но
