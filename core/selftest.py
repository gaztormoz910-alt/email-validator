...
ПРО ВЫВОД. У сборки с `console=False` `sys.stdout` и `sys.stderr` равны None.
Любой `print` внутри чужой библиотеки на таком потоке падает, поэтому вывод
перенаправляется в файл ПЕРВЫМ действием, до единого импорта движка.
...
def run(путь_лога):
    ...
    # ПЕРВЫМ делом: у сборки без консоли потоки равны None, и любой print
    # внутри чужой библиотеки убил бы прогон раньше, чем начал.
    sys.stdout = лог
    sys.stderr = лог
        ...
        try:
            for адрес in адреса:
                хранилище.append(адрес, "Unverified", "selftest", "",
                                 {"score": балл})
            всего = sum(хранилище.counts().values())
            шаг(всего >= len(адреса),
                "в хранилище легло строк: %d" % всего)
        finally:
            хранилище.close()
