...
# Зоны, которых НЕ СУЩЕСТВУЕТ в публичном DNS и существовать не может.
#
# Это не список подозрительных доменов, а факт из стандартов: перечисленные
# зоны зарезервированы и в корень интернета не делегированы. Домен в такой
# зоне не примет почту НИКОГДА — ни сегодня, ни завтра, ни с другого прокси.
# Значит это единственный вид приговора, который выносится честно и без
# единого запроса в сеть.
#
#   RFC 2606  — .test .example .invalid .localhost
#   RFC 6761  — они же как «домены специального назначения»
#   RFC 6762  — .local (mDNS, живёт только внутри локальной сети)
#   RFC 8375  — home.arpa
#   RFC 7686  — .onion (Tor; обычным SMTP туда не доставить)
#   RFC 9476  — .alt
#   ICANN 2024 — .internal, закреплён за частными сетями
#
# Остальные — зоны из локальных сетей, которые ICANN рассматривал и НЕ
# делегировал (.corp, .home, .mail, .lan). Их пишут в роутерах и корпоративных
# доменах Windows, и в интернете они не резолвятся.
#
# ЧЕГО ЗДЕСЬ НАМЕРЕННО НЕТ. Доменов второго уровня example.com/.org/.net тут
# нет, хотя RFC 2606 резервирует и их: они в DNS существуют, MX-записи у них
# нет, и текущая проверка и так вернёт про них «нет MX». Дублировать факт
# двумя разными способами незачем — разойдутся.
RESERVED_TLDS = frozenset({
    "test", "example", "invalid", "localhost",
    "local", "internal", "alt", "onion",
    "localdomain", "lan", "home", "corp", "mail",
    "intranet", "private", "domain", "workgroup",
})

# Полные суффиксы, а не одна метка: home.arpa зарезервирован целиком, а
# сама зона .arpa живая и настоящая.
RESERVED_SUFFIXES = ("home.arpa", "in-addr.arpa", "ip6.arpa")


def reserved_zone(domain):
    """Зарезервированная зона домена, либо пусто.

    Ответ — это ИМЯ ЗОНЫ, а не «да/нет»: причину отказа владелец должен
    прочитать словами, а не догадываться, за что адрес похоронили.
    """
    if not isinstance(domain, str):
        return ""
    низ = domain.strip().lower().rstrip(".")
    if not низ:
        return ""
    for суффикс in RESERVED_SUFFIXES:
        if низ == суффикс or низ.endswith("." + суффикс):
            return суффикс
    зона = низ.rsplit(".", 1)[-1]
    return зона if зона in RESERVED_TLDS else ""
