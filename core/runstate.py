...
class RunState:
    ...
    def reset_seen(self):
        """Забывает «этот адрес уже встречался», сохраняя журнал сделанного.

        ЗАЧЕМ ОТДЕЛЬНО ОТ clear(). Таблицы `seen` и `done` отвечают на разные
        вопросы, и путать их сроки жизни оказалось дорого.

        `seen` — дедуп ОДНОГО прохода по файлу: «эту строку я уже брал в
        работу». `done` — журнал вердиктов: «по этому адресу ответ получен».

        Прерванный прогон оставляет их расходящимися: адрес, который фидер
        прочитал, но который не успел получить вердикт (лежал в очереди,
        когда нажали «Стоп»; ждал перепроверки по серому списку; выпал по
        сбою), есть в `seen` и отсутствует в `done`. При продолжении фидер
        спрашивал СНАЧАЛА `seen` — получал «уже встречался» — и пропускал
        адрес как ДУБЛЬ, ни разу не заглянув в журнал. До проверки такой
        адрес не доходил никогда и в выгрузке не появлялся вовсе, а в логе
        числился схлопнутым дублем. То есть тише всего терялись ровно те
        адреса, ради которых продолжение и затевается.

        Поэтому новый проход начинается с чистым `seen`: дедуп внутри прохода
        отстраивается заново за один проход по файлу, а `done` переживает
        перезапуск и продолжает пропускать по-настоящему проверенное.
        """
        if self._conn is None:
            return
        with self._lock:
            ...
            self.seen_count = 0
            try:
                self._conn.execute("DELETE FROM seen WHERE run_id = ?",
                                   (self.run_id,))
                self._conn.commit()
            except Exception:
                self._close_quietly()
