...
class GravatarChecker:
    ...
    def has_gravatar(self, email: str) -> bool:
        ...
        """
        # Мусор на входе — это «аватарки нет», а не падение. Метод зовётся
        # на каждый адрес из рабочих потоков, где исключение проглатывает
        # except уровнем выше: адрес молча выпал бы из выдачи, а счётчик его
        # засчитал бы. Найдено построчным чтением; фаззинг набора до этого
        # метода не доходил, потому что проверяет функции модулей, а не
        # методы классов.
