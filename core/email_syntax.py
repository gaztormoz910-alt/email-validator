...
# Та же грамматика для имени ящика НЕ ИЗ ASCII (RFC 6531 §3.3): к atext
# добавлен весь диапазон вне ASCII, всё остальное — dot-atom без изменений.
#
# Зачем отдельная регулярка, а не «раз не ASCII, значит верим». Ветка для
# не-ASCII проверяла только точки, и любой знак, которым разделяют колонки в
# файле, проходил насквозь: строка `Иван,Мужской,Россия,ivan@gmail.com`
# признавалась ОДНИМ законным адресом. Загрузчик на этом основании не дробил
# её вовсе и отдавал наружу целиком — то есть у владельца пропадали и имя, и
# пол, и страна, а вместо адреса уезжала вся строка. С латиницей тот же файл
# разбирался верно: там работала общая регулярка RFC 5322.
#
# SMTPUTF8 расширяет НАБОР ЗНАКОВ, а не грамматику. Запятая, точка с запятой
# и двоеточие не входят в atext ни в ASCII, ни вне его.
_UTF8_ATEXT = _ATEXT + '\u0080-\U0010ffff'
_UTF8_LOCAL_RE = re.compile(
    r'^[' + _UTF8_ATEXT + r']+(?:\.[' + _UTF8_ATEXT + r']+)*$')
...
def validate_email_syntax(email: str) -> bool:
    ...
    if domain_literal_ip(domain) is not None:
        ...
        if not local.isascii():
            return bool(_UTF8_LOCAL_RE.match(local))
    ...
    #
    # Но СТРУКТУРА имени ящика проверяется и здесь — своей грамматикой
    # (_UTF8_LOCAL_RE), а не одними правилами про точки. Пока правил было
    # два с половиной, ветка пропускала всё, чего в них не назвали: сначала
    # `.ведущая-точка@corp.test`, потом — целую строку с запятыми,
    # `Иван,Мужской,Россия,ivan@gmail.com`. Второе стоило владельцу трёх
    # колонок данных на кириллических файлах и было тише первого: адрес не
    # объявлялся битым, он просто уезжал наружу вместе со всей строкой.
    #
    # Точки и разделители — это dot-atom из RFC 5322 §3.4.1, и SMTPUTF8 их
    # не отменяет; он расширяет набор знаков, а не грамматику.
    if not local.isascii():
