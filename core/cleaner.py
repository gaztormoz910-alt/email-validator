...
class EmailCleaner:
    ...
    def __init__(self):
        # Проверка домена по DNS, если её кто-то поставил. Нужна одному
        # месту — починке склейки (см. _strip_tld_tail). Пока её нет,
        # очистка работает как прежде: она обязана оставаться пригодной
        # без сети, иначе её нельзя ни проверить, ни применить офлайн.
        self._проверка_домена = None
    ...
    def _strip_tld_tail(self, domain: str) -> str:
        ...
        for длина in range(len(last_label) - 1, 1, -1):
            ...
            if зона[-1] == хвост[0]:
                continue
            почищенный = domain[:len(domain) - len(хвост)]
            # ПОСЛЕДНИЙ ПРЕДОХРАНИТЕЛЬ — и единственный, основанный не на виде
            # строки, а на факте. Если конвейер дал нам проверку, спрашиваем:
            # принимает ли почту то, что получится? Только «да» разрешает
            # подмену. Так `mail.ruxxx` по-прежнему чинится до `mail.ru`
            # (записи есть), а `honda.carpoi` остаётся собой (`honda.car` не
            # отвечает) — и вердикт выносится про адрес, который загрузили.
            #
            # «Спросить не удалось» (None) приравнено к «не чиним»: подменять
            # ящик без доказательства нельзя. Ложного приговора это не
            # создаёт — загруженный домен получит тот же честный ответ DNS, а
            # «не смогли спросить» в конвейере значит «не проверено», а не
            # «мёртв».
            if self._проверка_домена is not None:
                try:
                    отвечает = self._проверка_домена(почищенный)
                except Exception:
                    отвечает = None
                if отвечает is not True:
                    return domain
            return почищенный
        return domain
    ...
    def установить_проверку_домена(self, проверка):
        """Ставит функцию, которая говорит, принимает ли домен почту.

        Возвращаемое значение проверки трактуется в трёх состояниях:
        True — домен отвечает и почту принимает; False — записей нет; None —
        спросить не удалось. Чинить склейку разрешено ТОЛЬКО на True.

        Зачем это здесь. Признак «к зоне приклеен мусор» одинаково подходит
        и настоящей склейке (`mail.ruxxx`), и чужому домену (`honda.carpoi`,
        где после реза получается `honda.car` — тоже правдоподобное имя).
        Отличить их строкой нельзя, а фактом можно: у `mail.ru` почтовые
        записи есть, у `honda.car` их нет.

        Ставит эту проверку конвейер, когда сеть уже поднята. Без неё
        очистка ведёт себя как раньше, и все её офлайновые проверки
        продолжают описывать её же поведение.
        """
        self._проверка_домена = проверка if callable(проверка) else None
