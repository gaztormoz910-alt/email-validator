...
def _validator(**kw):
    """Валидатор с ЧИСТОЙ долговременной памятью.

    ЗАЧЕМ ИЗОЛЯЦИЯ. Память общая для машины: там лежат catch-all домены,
    профили прокси и СУТОЧНЫЙ СЧЁТ НАГРУЗКИ на каждый выходной IP. Для
    программы это правильно — нагрузка обязана переживать перезапуск. Для
    оракула это яд.

    Замерено 11.09.2026: проверка гео-маршрутизации краснела, и выглядело это
    как регрессия кода. На деле у подставного прокси 1.1.1.1 в общей памяти
    накопилась нагрузка 1602 при пороге 800, и он отсеивался как перегруженный
    ЕЩЁ ДО сравнения стран. Код был исправен; врала проверка, мерившая вчерашние
    счётчики.

    Набор тестов такую изоляцию делает давно (tests/conftest.py), а этот
    оракул запускается отдельно и её не имел.
    """
    import tempfile
    ...
    global _ПАМЯТЬ_ИЗОЛИРОВАНА
    if not _ПАМЯТЬ_ИЗОЛИРОВАНА:
        папка = tempfile.mkdtemp(prefix="gaps-longterm-")
        longterm.DEFAULT_PATH = os.path.join(папка, "longterm.sqlite")
        _ПАМЯТЬ_ИЗОЛИРОВАНА = True
    return NetworkValidator(timeout=1, **kw)


# Подменяем путь памяти один раз на запуск: разные проверки внутри одного
# прогона должны видеть одну и ту же чистую базу, а не каждая свою.
_ПАМЯТЬ_ИЗОЛИРОВАНА = False
...
def check_monoliths_still_open():
    """Обратная проверка для ОТКРЫТОГО изъяна: монолиты всё ещё есть.

    Открытая строка обязана оставаться правдой так же, как закрытая. Если
    модули разберут, эта проверка покраснеет и потребует вернуть строке
    статус «закрыт» — работа не должна пропадать незасчитанной.
    """
    return not check_monoliths()


OPEN_CHECKS = {
    "monoliths": check_monoliths_still_open,
}
