...
def test_many_per_line_all_addresses_survive():
    """Второй адрес больше не пропадает."""
    for строка, ждём in (("a@x.com, b@y.com", 2),
                         ("a@x.com b@y.com c@z.com", 3),
                         ("a@x.com;b@y.com", 2)):
        вышло = загрузить(строка)
        assert len(вышло) == ждём, "%r -> %s" % (строка, вышло)
