...
# Помечен как медленный ПО ЗАМЕРУ (pytest --durations): тесты здесь грузят
# модели имён и ходят по сети, каждый — секунды. Быстрый круг:
...
# Классы TestBackgroundThreadDoesNotStarveTheWindow и
# TestClosedWindowIsNotAnError удалены 06.09.2026 вместе со старым окном:
# оба поднимали настоящий Tk и мерили тики его цикла.
#
# Что из них потеряно, а что нет. Про GIL: сам механизм (breathe_every в
# scan_base_providers) на месте и покрыт tests/test_owner_logs.py — там он
# проверяется вызовом, без окна. Про `_ui_call`: это был метод именно
# старого окна, у веб-окна такого нет, и терять нечего.
