...
class TestMemoryIsBounded(unittest.TestCase):
    ...
    ROWS = 60_000
    # Потолок. В памяти обязаны остаться только позиции — восемь байт на
    # строку в array('q'), то есть 0.48 МБ на эти шестьдесят тысяч, — плюс
    # буфер записи на пятьсот строк.
    #
    # ЧИСЛО ВЗЯТО ИЗ ЗАМЕРА, А НЕ ИЗ ГОЛОВЫ. Замерено 11.09.2026 в чистом
    # процессе, три прогона подряд дали одно и то же: 0.698 МБ. Те же строки
    # словарями в ОЗУ — 28.0 МБ, то есть в сорок раз больше.
    #
    # Прежние 12 стояли ровно посередине между этими двумя числами и почти
    # ничего не ловили: до них расход мог вырасти в семнадцать раз, и проверка
    # молчала бы. Три — это четырёхкратный запас над измеренным и восьмикратный
    # отрыв от поломки, которую проверка сторожит.
    CAP_MB = 3
    ...
    def test_positive_control_in_memory_store_is_heavier(self):
        ...
        # И ГЛАВНОЕ: потолок обязан лежать МЕЖДУ этими двумя числами. Потолок
        # выше поломки не ловит её вовсе — именно так было с прежними 12 МБ
        # после того, как замер починили и настоящий расход упал до 0.7 МБ.
        self.assertGreater(
            in_ram, self.CAP_MB,
            f"потолок {self.CAP_MB} МБ выше самой поломки ({in_ram:.1f} МБ) — "
            "он ничего не сторожит")
        self.assertLess(
            on_disk, self.CAP_MB,
            f"потолок {self.CAP_MB} МБ ниже обычного расхода ({on_disk:.2f} МБ)")
