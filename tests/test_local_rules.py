...
class TestNoFalseInvalid(unittest.TestCase):
    ...
    def test_no_false_invalid_positive_control(self):
        ...
        # `a b@gmail.com` из этого списка УБРАН 06.09.2026 по решению
        # владельца: пробел внутри адреса теперь склеивается (`a b@` ->
        # `ab@`), то есть это тоже ремонт, а не брак — как и двойная точка
        # строкой выше. Контроль от этого не ослаб: шесть заведомо
        # неисправимых форм остались, и любая из них ловит поломку
        # синтаксической проверки.
        for bad in ("@gmail.com", "no-at-sign", "a@b", "a@", "@",
                    "x" * 65 + "@gmail.com", "ivan@почта"):
