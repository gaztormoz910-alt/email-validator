...
def reset_app(app):
    ...
    # Терминал тоже виджет и тоже копит между тестами. Проверки, которые
    # смотрят на ПОРЯДОК строк, на чужом хвосте дают бессмысленный результат:
    # строка «завершена» из предыдущего теста оказывается раньше результатов
    # текущего.
    try:
        app.terminal_box.configure(state="normal")
        app.terminal_box.delete("1.0", "end")
        app.terminal_box.configure(state="disabled")
    except Exception:
        pass
