...
@pytest.mark.xfail(strict=True, reason=(
    "ИЗВЕСТНЫЙ НЕЗАКРЫТЫЙ ДЕФЕКТ, а не недосмотр. `207144@honda.carpoi` в "
    "файле владельца — последний токен ОБРЕЗАННОЙ строки "
    "(`...skhonda2@sunnyking.com 207144@honda.carpoi`), и настоящий домен "
    "неизвестен. Очистка режет метку `carpoi` по зоне `ca` и выдаёт "
    "`honda.ca` — существующий домен Honda Канада, которого в файле не было. "
    "Ни один из четырёх признаков, закрывших остальные двенадцать подмен "
    "(слово в метке, короткий хвост, метка в букве от длинной зоны, разрыв "
    "цепочки одинаковых букв), этот случай не отделяет, а пятого, который не "
    "задел бы настоящие склейки `yandex.rublahblah` и `corp.deSpam`, на "
    "данных владельца не нашлось. Настоящее решение — не резать молча вовсе, "
    "а предлагать починку рядом с загруженным адресом, как уже сделано для "
    "опечаток; это меняет вид таблицы и решается владельцем. "
    "strict=True: если поведение изменится в любую сторону, проверка упадёт, "
    "и дефект не исчезнет из виду молча."))
def test_d02_обрезанная_метка_всё_ещё_подменяет_домен():
    """Единственная из тринадцати подмен, которая осталась незакрытой."""
    стало = (EmailCleaner().clean_email("207144@honda.carpoi")
             or "").rsplit("@", 1)[-1]
