...
def test_no_offline_verdict_without_proof(rows):
    ...
    Число здесь — ИЗМЕРЕННОЕ свойство базы, а не догадка; если базу заменят
    на другую, его надо перемерить, а не подогнать.

    Было ноль, стало два, и причина не в ослаблении проверки. Две строки в
    базе записаны как `u003e+2@gmail.com` — это `>+2@gmail.com`, у которого
    потерялся обратный слэш экранирования. Пока вход этого не видел, строка
    выглядела обычным адресом и уезжала спрашивать сервер. Теперь обёртка
    снимается, и под ней оказывается `+2@gmail.com`: у Gmail плюс отделяет
    метку от имени, и имени тут нет вовсе. Адресовать в таком ящике нечего.

    Проверка от этого не ослабла: список сверяется ПОИМЁННО, и любой третий
    адрес, осуждённый без сети, её уронит.
    """
    cleaner = EmailCleaner()
    condemned = []
    for raw in rows:
    ...
    # Ровно эти два, и ни одного другого.
    ИЗВЕСТНЫЕ = {"+2@gmail.com", "+3@gmail.com"}
    неожиданные = [c for c in condemned if c.split(":")[0] not in ИЗВЕСТНЫЕ]
    assert not неожиданные, (
        "%d адресов получили Invalid без единого запроса к серверу: %s"
        % (len(неожиданные), неожиданные[:10]))
    осуждённые = {c.split(":")[0] for c in condemned}
    assert осуждённые == ИЗВЕСТНЫЕ, (
        "набор осуждённых без сети изменился: ждали %s, получили %s"
        % (sorted(ИЗВЕСТНЫЕ), sorted(осуждённые)))
