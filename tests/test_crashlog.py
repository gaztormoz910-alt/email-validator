...
def test_announce_counts_a_recent_crash_but_says_nothing_in_the_window(log):
    """Прошлая авария считается, но человеку в окно не выводится.

    ПУТЬ ЭТОГО ТРЕБОВАНИЯ. Сначала программа кричала «[DEAD] сломалась» и
    перечисляла каждую запись красной строкой — красное полотно сразу после
    запуска. Потом это стало одной строкой с меткой INFO. 11.09.2026 владелец
    сказал прямо: «не надо мне лишние логи отображать», — и это его решение.

    Проверка не ослаблена, а перенацелена: раньше она сторожила ФОРМУЛИРОВКУ
    сообщения, теперь сторожит его ОТСУТСТВИЕ в окне при сохранной записи на
    диске. Разбирать аварию по-прежнему есть по чему.
    """
    ...
    api = ValidatorApi()
    # Считает — значит видит запись, и «в окне пусто» не оттого, что искать
    # было нечего.
    assert _announce_past_crashes(api) >= 1

    lines = " ".join(row["text"] for row in api.log_tail()["log"])
    assert "сбо" not in lines.lower(), "лишняя строка про прошлый сбой вернулась"
    assert crashlog.crash_log_path() not in lines, "путь к журналу снова в окне"
    assert "[DEAD]" not in lines, "прошлая авария помечена как текущая"

    # И главное: сама запись на диске никуда не делась.
